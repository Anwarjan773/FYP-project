...
// Get values from URL (if coming from doctor profile)
$preselect_doctor_id = $_GET['doctor_id'] ?? '';
$preselect_deptt_id = $_GET['deptt_id'] ?? '';
...
<?php  include '../includes/navbar.php'; ?>
  <div class="container my" style="width:800px">
      ...
      <form method="POST" action="" >
        <div class="row ">
          ...
          <select name="department_id" id="departmentSelect" class="form-select" required>
                <option value="" disabled <?= empty($preselect_deptt_id) ? 'selected' : '' ?>>Choose Department</option>
                <?php
                mysqli_data_seek($departments, 0); // rewind for reuse
                while ($dept = mysqli_fetch_assoc($departments)):
                  $selected = ($preselect_deptt_id == $dept['deptt_id']) ? 'selected' : '';
                ?>
                  <option value="<?= $dept['deptt_id'] ?>" <?= $selected ?>>
                    <?= $dept['deptt_name'] ?>
                  </option>
                <?php endwhile; ?>
              </select>
          ...
          <div class="col-md-6">
           ...
           <select name="doctor_id" id="doctorSelect" class="form-select" required>
            <option value="" disabled <?= empty($preselect_doctor_id) ? 'selected' : '' ?>>Choose Doctor</option>
            <?php foreach ($doctorList as $doc): ?>
              <?php
                // Show only doctors that belong to the selected department
                if (!empty($preselect_deptt_id) && $doc['deptt_id'] != $preselect_deptt_id) {
                  continue;
                }

                $selected = ($preselect_doctor_id == $doc['doctor_id']) ? 'selected' : '';
              ?>
          ...
          <div class="col-12">
            ...
            <textarea name="message" class="form-control" rows="2" placeholder="Describe symptoms or concerns..."></textarea>
          </div>
